...
cmd({
  ...
  try {
    // فقط چک می‌کنیم گروه هست و بات ادمین هست
    if (!isGroup || isAdmins || !isBotAdmins) return;

    // اگر antiviewonce فعال بود و antidelete در حالت same یا inbox بود
    if (config.ANTI_VIEWONCE === "true" && (config.ANTI_DEL_PATH === "same" || config.ANTI_DEL_PATH === "inbox")) {
