...
const AntiDelete = async (conn, updates) => {
  for (const update of updates) {
    if (update.update.message === null) {
      // ✅ جلوگیری از واکنش به حذف پیام‌های خود ربات
      if (update.key.participant === conn.user.id) continue;
      ...
      const actionType = getActionByMessageType(mek.message); // نوع اکشن

      // استخراج متن یا کپشن پیام
      let messageText = '';
